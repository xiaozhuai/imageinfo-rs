...
fn copy_slice<R>(
    dst: &mut [R], dst_offset: usize, dst_size: usize,
    src: &[R], src_offset: usize, src_size: usize,
) where R: Copy {
    dst[dst_offset..(dst_offset + dst_size)].copy_from_slice(&src[src_offset..(src_offset + src_size)]);
}
...
impl<R> ReadInterface<R> where R: BufRead + Seek {
    pub fn read(&mut self, offset: usize, size: usize) -> ImageInfoResult<RawBuffer> {
        ...
        if offset + size <= self.header_cache.len() {
            copy_slice(buffer.data.as_mut_slice(), 0, size, self.header_cache.as_slice(), offset, size);
        } else if offset < self.header_cache.len() && self.header_cache.len() - offset >= (self.header_cache.len() / 4) {
            let head = self.header_cache.len() - offset;
            copy_slice(buffer.data.as_mut_slice(), 0, head, self.header_cache.as_slice(), offset, head);
